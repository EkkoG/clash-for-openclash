...
latest_version=$(curl -s https://api.github.com/repos/Dreamacro/clash/releases/latest | grep tag_name | cut -d '"' -f 4 | tr -d 'v')
...
if [ "$current_version" = "$latest_version" ]; then
    ...
    wget https://github.com/Dreamacro/clash/archive/refs/tags/v$latest_version.tar.gz -O clash.meta.tar.gz
